...
router.post('/class', async (req, res) => {
    ...
    const newCl = await new Classes({ year: new Date(y.toString() + "-01-01T00:00:00.000+00:00"), num: ns, city:city, department:department });
    newCl.save()
    num = parseInt(ns)+1
    res.json({num: num})
})
...
router.get('/getAllDepartmentsS', async (req, res) => {
    ...
    console.log(deps)
    deps.map(x => { depArr.push(x.nameShort) })
    console.log(depArr);
    res.json({ deps: depArr })
})
...
router.get('/getAllCitysS', async (req, res) => {
    ...
    citys.map(x => { cityArr.push(x.nameShort) })
...
router.get('/getAllClassesNL', async (req, res) => {
    ...
    classes.forEach(x => {
        ...
        var num = x.num.toString()
        num = num.length == 1 ? "0" + num : num
        clsNL.push(x.department!=null?(x.department.nameShort + "-" + x.city.nameShort + "-" + year + "-" + num):null)
        clsIds.push(x.department!=null?x._id:null)
    })
    res.json({ classes: clsNL, ids: clsIds })
})
...
router.post('/getClassesOfDocent', async (req, res) => {
    ...
    if (doz != "") {
        ...
        classes.forEach(klasse => {
            ...
            var nameLong = klasse.department!=null?klasse.department.nameShort + "-" + klasse.city.nameShort + "-" + year + "-" + num:null
